        ...
        static void Main(string[] args)
        {
            ...
            while (!command.Equals("end of dates"))
            {
                ...
                {// if it contains, check if its list DatesAttended contains such date
                    ...
                    {
                        //if (!myMentorGroup.First(s => s.Name == inputInfo[0]).DatesAttended.Contains(DateTime.ParseExact(inputInfo[i], "dd/MM/yyyy", null)))
            ...
            while (!command.Equals("end of comments"))
            {
                ...
                {// if it contains, check if its list CommentsReceived contains such comment
                    if (inputLength > 1)
                    {
                        //if (!myMentorGroup.First(s=>s.Name == inputInfo[0]).CommentsReceived.Contains(inputInfo[1]))
